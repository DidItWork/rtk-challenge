...
class StereoCameraCalibration:
    ...
    def compute_lane_deviation(self): # Parallel Stereo Cameras; Epipolar Geometry
        ...
        left_camera_frame = self.get_webcam_frame()
        lanes = self.detect_lane(left_camera_frame) # limits of the track; left & right lane image
    ...
    def get_webcam_frame(self): # WebCam
        ...
        if self.zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
        	...
        	self.zed.retrieve_image(camera_frame, sl.VIEW.LEFT)       
        ...
        camera_frame = cv2.cvtColor(camera_frame.get_data(), cv2.COLOR_RGBA2GRAY)

        #camera_frame = cv2.cvtColor(camera_frame, cv2.RGB2BGR)

        return camera_frame
    
    def find_farthest_pixel(self, lines):
        farthest_pixel = None
        max_distance = -1
        ...
        for line in lines:
            for x1, y1, x2, y2 in line:
                ...
                if distance > max_distance:
                    ...
                    farthest_pixel = (x1, y1) if y1 > y2 else (x2, y2)

        return farthest_pixel
    
    def find_nearest_pixel(self, lines):
        nearest_pixel = None
        ...
        for line in lines:
            for x1, y1, x2, y2 in line:
                ...
                if distance < min_distance:
                    ...
                    nearest_pixel = (x1, y1) if y1 > y2 else (x2, y2)
    ...
    def segment_lines(self, lines, image_width):
        left_lines = []
        right_lines = []
    ...
    def display_image(self):
        center_coordinates = (int(self.image_size.width/2),int(self.image_size.height/2))

        camera_frame = self.get_webcam_frame()
        lines = self.line_detection(camera_frame)
        
        left_lines, right_lines = self.segment_lines(lines, self.image_size.width)

        farthest_left_pixel = self.find_farthest_pixel(left_lines)
        farthest_right_pixel = self.find_farthest_pixel(right_lines)
        nearest_left_pixel = self.find_nearest_pixel(left_lines)
        nearest_right_pixel = self.find_nearest_pixel(right_lines)

        # print(farthest_left_pixel)

        if farthest_left_pixel and farthest_right_pixel and nearest_left_pixel and nearest_right_pixel is not None:
            farthest_center_pixel = ((farthest_left_pixel[0]+farthest_right_pixel[0])/2, (farthest_left_pixel[1]+farthest_right_pixel[1])/2)
            nearest_center_pixel = ((nearest_left_pixel[0]+nearest_right_pixel[0])/2, (nearest_right_pixel[1]+nearest_right_pixel[1])/2)

        # Draw detected lines on a copy of the original image
        lines_image = np.copy(camera_frame)
        # print("Camera Frame Shape", lines_image.shape)
        if left_lines and right_lines is not None:
            for line in left_lines:
                x1, y1, x2, y2 = line[0] # list within a list
                cv2.line(lines_image, (x1, y1), (x2, y2), (0, 255, 0), 3) # image, start_point, end_point, color (BGR), thickness
           
            for line in right_lines:
                x1, y1, x2, y2 = line[0] # list within a list
                cv2.line(lines_image, (x1, y1), (x2, y2), (0, 255, 0), 3) # image, start_point, end_point, color (BGR), thickness

        # Draw the farthest left & right pixel of the track and its centerline
        #cv2.circle(lines_image, farthest_left_pixel, radius=2, color=(255, 0, 0), thickness=10)
        #cv2.circle(lines_image, farthest_right_pixel, radius=2, color=(255, 0, 0), thickness=10)
        cv2.circle(lines_image, nearest_left_pixel, radius=2, color=(255, 0, 188), thickness=10)
        cv2.circle(lines_image, nearest_right_pixel, radius=2, color=(255, 0, 188), thickness=10)
        #cv2.line(lines_image, nearest_center_pixel, farthest_center_pixel, (209, 165, 69), 2)

        # Draw the centerpoint on the copy of the original image
        cv2.circle(lines_image, center_coordinates, radius=2, color=(0,0,255), thickness=-1) # negative thickness for filled circle
        # cv2.imwrite("lines.png", lines_image)
        # lines_image = cv2.resize(lines_image, (640, 480))
    ...
    def algorithm_1(self):
        """
        Algorithm #1: Lane Detection with ONLY Left Camera
        > Get left camera frame
        > Apply line detection on image frame
        > Get the pixel coordinates of the left limit and right limit of the track 
            (Note: high x-coordinates and low/zero y-coordinates)
        > Determine the centerline of the track
        > Determine the centerpoint of the image (i.e., assume origin of car)
        > Compute deviation of centerline from centerpoint
        """
        left_camera_frame, right_camera_frame = self.get_camera_frame()
        left_lane, right_lane = self.detect_lane(left_camera_frame, right_camera_frame) # limits of the track; left & right lane image
        left_bounding_pixel = None # left limit pixel on the x-axis
        right_bounding_pixel = None # right limit pixel on the x-axis 
        center_point = (left_bounding_pixel + right_bounding_pixel) / 2 # centerline of the track
        offset_centerline = self.car_origin[0] - center_point # positive_offset = steer left; negative_offset = steer right
        offset_right_limit = right_bounding_pixel - self.car_origin[0]
        offset_left_limit = self.car_origin[0] - left_bounding_pixel

        return offset_centerline, offset_right_limit, offset_left_limit
...
class BuggyCar(Node):
    ...
    def lane_detection_callback(self): # MAIN PROGRAM: line detection of center line between boundary lanes
        ...
        self.cmd.linear.x = self.forward_speed # constant forward speed
